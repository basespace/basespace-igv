...
public class BusyGlassPaneRenderer extends javax.swing.JPanel
{
    ...
    public BusyGlassPaneRenderer(RenderContext context,String initialText)
    {
        this(context.getPanel(), 100, initialText, true);
    ...
    public BusyGlassPaneRenderer(Component c, int progressMax)
    {
        this(c, progressMax, null);
    }

    public BusyGlassPaneRenderer(Component c, int progressMax, String initialText)
    {
        this(c, progressMax, null, true);
    }

    public BusyGlassPaneRenderer(Component c, int progressMax, String initialText, boolean blockInput)
    {
    ...
    public void process(List<ProgressReport> chunks)
    {
        ...
        {
            ProgressReport progressReport = (ProgressReport) chunk;
            progress = progressReport.getProgress();
            text = progressReport.getText();
        }
    ...
    private Rectangle getOverComponentBounds()
    {
        ...
        {
            loc = SwingUtilities.convertPoint(overComponent,renderContext.getVisibleRect().getLocation(), pane);
            return new Rectangle(loc.x, loc.y, (int)renderContext.getVisibleRect().getWidth(), 
                    (int)renderContext.getVisibleRect().getHeight());
        }
        else
        {
    ...
    protected class EventCapture implements AWTEventListener
    {
        ...
        public void eventDispatched(AWTEvent event)
        {
            if (event instanceof MouseEvent)
            {
                ...
                if (!overComponent.isShowing())return;
