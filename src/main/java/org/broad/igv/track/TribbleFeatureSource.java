...
public class TribbleFeatureSource implements org.broad.igv.track.FeatureSource
{
    ...
    protected CachingFeatureReader reader;
    protected DataSource coverageSource;
    protected boolean isVCF;
    protected Genome genome;

    /**
     * Map of IGV chromosome name -> source name
     */
    protected Map<String, String> chrNameMap = new HashMap();
    protected int featureWindowSize;
    protected Object header;
    protected Class featureClass;

    public TribbleFeatureSource()
    {
    ...
    protected void initCoverageSource(String covPath)
    {
    ...
    public CloseableTribbleIterator<Feature> getFeatures(String chr, int start, int end) throws IOException
    {
        ...
        //log.info("Get features from " + reader.getClass().getName() + " for chr " + chr + ", start=" + start + ",end=" + end);
        return reader.query(seqName, start, end);
    }

    /**
     * Return coverage values overlapping the query interval. At this time
     * Tribble sources do not provide coverage values
     * 
     * @param chr
     * @param start
     * @param end
    ...
    {
        return coverageSource == null ? null : coverageSource.getSummaryScoresForRange(chr, start, end, zoom);
    }
     ...
     * @param reader
     */
    protected void initFeatureWindowSize(FeatureReader reader)
    {
