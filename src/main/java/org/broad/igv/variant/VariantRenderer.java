...
public class VariantRenderer { //extends FeatureRenderer {
    ...
    public void renderSiteBand(Variant variant,
        ...
        boolean drawDebugString = false;
        if (remainderHeight > 0) {
            Graphics2D g = context.getGraphic2DForColor(refColor);
            g.fillRect(rectX, bottomY - alleleBarHeight - remainderHeight, rectWidth, remainderHeight);
            Color temp = g.getColor();
            g.setColor(Color.BLACK);
            int textX = rectX;
            int textY = bottomY - alleleBarHeight - remainderHeight;
            textY = textY - 5;
            if (drawDebugString)g.drawString(String.valueOf(variant.getStart()),textX,textY);
            textY = textY + 20;
            textX = textX + 10;
            if (drawDebugString)g.drawString(String.valueOf(rectX),textX,textY);
            g.setColor(temp);
        }

        if (alleleBarHeight > 0) {
            Graphics2D g = context.getGraphic2DForColor(alleleColor);
            g.fillRect(rectX, bottomY - alleleBarHeight, rectWidth, alleleBarHeight);
            Color temp = g.getColor();
            g.setColor(Color.BLACK);
            int textX = rectX;
            int textY = bottomY - alleleBarHeight - remainderHeight;
