...
public class AlignmentReaderFactory
{
    ...
    public static AlignmentReader getReader(ResourceLocator locator, boolean requireIndex) throws IOException
    {
        ...
        else if (pathLowerCase.endsWith("sorted.txt") || pathLowerCase.endsWith(".aligned")
                || pathLowerCase.endsWith(".aligned.txt") || pathLowerCase.endsWith("bedz")
                || pathLowerCase.endsWith("bed") || pathLowerCase.endsWith("psl") || pathLowerCase.endsWith("pslx"))
        {
            reader = new GeraldReader(samFile, requireIndex);
        }
        //BASESPACE
        else if (locator.getType() != null && locator.getType().endsWith(".bam"))
        {
            try
            {
                reader = new BaseSpaceBAMHttpReader(locator);
            }
            ...
            while ((nextLine = reader.readLine()) != null)
            {
                ...
                if (nextLine.startsWith("#replace"))
                {
                    ...
                    if (tokens.length == 2)
                    {
                        ...
                        if (kv.length == 2) replacements.put(kv[0], kv[1]);
                    }
                }
                else
                {
